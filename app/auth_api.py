...
def create_job_posting(
    ...
    job = Job(
        ...
        source_url=f"https://joborra.com/jobs/{job_data.title.lower().replace(' ', '-')}-{int(datetime.now().timestamp())}",
...
def publish_job_draft(
    ...
    try:
        ...
        job = Job(
            ...
            international_student_friendly=draft.international_student_friendly,
            source_website="joborra.com",
            source_url=f"https://joborra.com/jobs/{draft.title.lower().replace(' ', '-')}-{draft_id}-{int(datetime.now().timestamp())}",

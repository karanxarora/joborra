          ...
          <div className="hidden md:flex items-center space-x-6">
            <nav className="flex items-center space-x-6">
              {(!isAuthenticated || user?.role === 'student') && (
                <DisabledLink>Browse Jobs</DisabledLink>
              )}
              {isAuthenticated && user?.role === 'student' && (
                <DisabledLink>Saved Jobs</DisabledLink>
              )}
              ...
              {isAuthenticated && user?.role === 'employer' && (
                <Link to="/employer/drafts" className="text-slate-600 hover:text-primary-600 transition-colors">
                  Job Drafts
                </Link>
              )}
                ...
                {menuOpen && (
                  <div className="absolute right-0 mt-2 w-72 bg-white border border-slate-200 rounded-lg shadow-xl py-2 z-50">
                    ...
                    {user?.role === 'student' && (
                      <>
                        <span
                          className="flex items-center gap-2 px-3 py-2 text-sm text-slate-400 cursor-not-allowed relative"
                    ...
                    {/* Job/Candidate Recommendations (placeholder) */}
                    <div className="px-3 py-2">
                      <div className="flex items-center gap-2 text-sm text-slate-700">
                        <Sparkles className="h-4 w-4 text-slate-500" />
                        {user?.role === 'employer' ? 'Candidate Recommendations' : 'Job Recommendations'}
                      </div>
                      <div className="text-[11px] text-slate-400 ml-6">
                        {user?.role === 'employer' 
                          ? '(Personalized candidates will appear here soon)' 
                          : '(Personalized jobs will appear here soon)'
                        }
                      </div>
                    </div>
